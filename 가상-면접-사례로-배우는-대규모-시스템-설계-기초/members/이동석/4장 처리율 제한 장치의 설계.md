...
- resilence4j 의 ratelimiter는 token bucket 기반의 알고리즘을 사용하는것으로 보임(추정)
  - https://resilience4j.readme.io/docs/ratelimiter#internals
    | 구분 | Resilience4j (Token Bucket 변형) | Leaky Bucket |
    | :--- | :--- | :--- |
    | **핵심 개념** | 토큰이 있으면 **즉시 처리** (Burst 허용) | 일정한 속도로만 처리 (Queue 기반) |
    | **유연성** | 짧은 시간 동안의 **트래픽 폭주를 수용** | 폭주 시에도 무조건 정해진 속도로 배출 |
    | **구현 목적** | **API 호출 제한** 및 서버 자원 보호 | 네트워크 트래픽 **쉐이핑(Smoothing)** |
